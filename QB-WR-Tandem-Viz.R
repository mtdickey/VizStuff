...
Dalton <- read.csv("DaltonTDs.csv", stringsAsFactors = F, header = F)
...
colnames(Dalton)[1:2] <- c("Receiver", "TotalTDs")

# Combine
All <- rbind(Brady, Rivers, Manning, Ryan, Stafford, Brees, Romo, Rodgers, Dalton)
...
All$together[which(All$Passer == "Dalton")] <- "Yes"
