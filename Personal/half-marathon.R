...
Times$Gender <- ifelse(Times$Gender == "M", "Male", ifelse(Times$Gender == "F", "Female","Other"))
...
Times %>% filter(Event == "Half") %>% 
  ...
  geom_vline(xintercept = Times$Time_min[which(Times$Name == "Michael Dickey")],
             color = "#fb6a4a", linetype = "dashed", size = 1.2) + 
  labs(x = "\nTime (minutes)", y = "Count of Finishers\n",
...
Times %>% filter(Event == "Half") %>%
  ...
  geom_hline(yintercept = Times$Time_min[which(Times$Name == "Michael Dickey")],
             color = "#fb6a4a", linetype = "dashed", size = 1.2) +
  guides(fill = F) + labs(title = "Distributions of Times at 2018 Potomac River Run Half Marathon",
                          subtitle = "Segmented gender and age group",
                          x = "Age Group",
                          y = "Time (minutes)") +
  theme_bw() + theme(axis.title = element_text(size = 14), axis.text = element_text(size = 13),
                     legend.text = element_text(size = 13))
